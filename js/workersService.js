...
async function saveWorker(){
  ...
  const paymentType = document.getElementById('workerPaymentType').value || null;
  const rate = Number(document.getElementById('workerRate').value) || null;
  const notes = document.getElementById('workerNotes').value.trim() || null;
  if (!name){ showToast('اسم الشخص مطلوب.', true); return; }
  showLoading(true);
  const payload = { name, payment_type: paymentType, rate, notes };
  const { error } = id
...
function resetWorkerForm(){
  ...
  document.getElementById('workerPaymentType').value = '';
...
function editWorker(id){
  ...
  document.getElementById('workerPaymentType').value = w.payment_type || '';
  ...
  el.innerHTML = state.workers.map(w => `
    <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px dashed var(--line);padding:8px 0;font-size:13px;">
      <div>
        ...
        ${w.payment_type ? (' — ' + (w.payment_type==='per_batch' ? 'بالتمييعة' : 'بالكيلو') + ' (' + Number(w.rate||0).toLocaleString() + ' ر.س)') : ' — <span style="color:var(--muted);">طريقة الحساب غير محددة بعد</span>'}
      </div>
...
async function calculateWorkerPaymentUI(){
  ...
  if (!data.payment_type){
    box.innerHTML = '<div class="hint" style="color:var(--debt);">لازم تحدد طريقة الحساب والسعر لهذا الشخص أول (من قسم "إضافة/تعديل شخص منفّذ" بالأعلى).</div>';
    return;
  }
  box.innerHTML = `
    <div class="card" style="background:#eef1e0;">
      ...
      <div style="margin-top:6px;">${data.payment_type === 'per_batch'
        ? ('مجموع التمييعات: ' + Number(data.total_batches).toLocaleString() + ' × ' + Number(data.rate).toLocaleString() + ' ر.س')
        : ('مجموع الكيلوجرامات: ' + Number(data.total_kg).toLocaleString() + ' × ' + Number(data.rate).toLocaleString() + ' ر.س')}</div>

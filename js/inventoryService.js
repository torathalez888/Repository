 ...
 * @param {string} p.note
 */
async function convertStock({ sourceItemId, sourceVariantId, sourceQtyConsumed, outputs, computeWaste, note, executorWorkerId, batchCount, workerPaymentType, workerRate }) {
  ...
  const { data, error } = await sb.rpc('fn_convert_stock', {
    ...
    p_worker_payment_type: workerPaymentType || null,
    p_worker_rate: workerRate || null,
...
/** يظهر/يخفي + يُلزم حقلي طريقة الحساب والسعر بس لو تم اختيار شخص منفّذ لهذي التمييعة */
function toggleConvWorkerPricingFields(){
  const hasWorker = !!document.getElementById('convExecutorWorker').value;
  const wrap = document.getElementById('convWorkerPricingWrap');
  if (wrap) wrap.style.display = hasWorker ? 'block' : 'none';
  if (!hasWorker){
    document.getElementById('convWorkerRate').value = '';
  }
}
...
async function submitStockConversion(){
  ...
  const workerPaymentType = executorWorkerId ? document.getElementById('convWorkerPaymentType').value : null;
  const workerRate = executorWorkerId ? (Number(document.getElementById('convWorkerRate').value) || null) : null;
  ...
  if (executorWorkerId && !workerRate){ showToast('حدّدت شخص منفّذ — لازم تدخل سعر أجره لهذي التمييعة بالذات.', true); return; }
  ...
  try {
    ...
    box.innerHTML = rows.map(r => {
      ...
      <div style="border-bottom:1px dashed var(--line);padding:8px 0;font-size:12.5px;">
        ...
        ${r.executor_worker_id ? `<div>👷 المنفّذ: ${escapeHtml(((state.workers||[]).find(w => w.id === r.executor_worker_id) || {}).name || 'شخص محذوف/موقوف')}
          ${r.worker_payment_type_snapshot ? (' — ' + (r.worker_payment_type_snapshot==='per_batch' ? 'بالتمييعة' : 'بالكيلو') + ': ' + Number(r.worker_rate_snapshot||0).toLocaleString() + ' ر.س') : ' — <span style="color:var(--debt);">سعر غير مسجّل</span>'}
          <button class="btn-sm btn-ghost write-action" style="padding:2px 8px;font-size:11px;" onclick="editConversionWorkerPricing(${r.id}, '${r.worker_payment_type_snapshot||''}', ${r.worker_rate_snapshot||0})">✏️ تعديل السعر</button>
        </div>` : ''}
